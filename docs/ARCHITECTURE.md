...
1. Read an HTML/text product-page snapshot from disk, fetch raw HTML with `snapshot`, or capture rendered HTML with the optional `agentshelf[render]` extra.
...
- Add polite site-map or product-list ingestion while keeping checkout and authenticated crawling out of scope.
